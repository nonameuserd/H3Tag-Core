...
exports.metricsCommand = new commander_1.Command("metrics").description("Get blockchain metrics and monitoring information");
// Get general metrics info command
exports.metricsCommand
    .command("info")
    .description("Get general blockchain metrics")
    .option("-t, --time-window <number>", "Time window in milliseconds (default: 1 hour)", "3600000")
    .action(async (options) => {
    try {
        const response = await api_1.api.get("/metrics", {
        ...
        console.error("Failed to get metrics:", error.response?.data?.error || error.message);
        process.exit(1);
    }
});
...
exports.metricsCommand
    ...
    try {
        ...
        console.error("Failed to get fee metrics:", error.response?.data?.error || error.message);
        process.exit(1);
    }
});
...
exports.metricsCommand
    ...
    try {
        ...
        console.error("Failed to get network metrics:", error.response?.data?.error || error.message);
        process.exit(1);
    }
});
...
exports.metricsCommand
    ...
    try {
        ...
        console.log(`Synced Headers: ${response.data.syncedHeaders}`);
        console.log(`Synced Blocks: ${response.data.syncedBlocks}`);
        const syncProgress = ((response.data.syncedBlocks / response.data.syncedHeaders) *
            100).toFixed(2);
        console.log(`Sync Progress: ${syncProgress}%`);
    }
    catch (error) {
        console.error("Failed to get sync status:", error.response?.data?.error || error.message);
        process.exit(1);
    }
});
...
exports.metricsCommand
    ...
    try {
        ...
        console.error("Failed to get volume metrics:", error.response?.data?.error || error.message);
        process.exit(1);
    }
});
