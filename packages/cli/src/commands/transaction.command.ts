...
export const transactionCommand = new Command("transaction").description(
...
transactionCommand
  ...
  .action(async (txId: string, options) => {
    try {
      const response = await api.get(`/api/v1/transactions/${txId}`);
...
transactionCommand
  ...
  .action(async (txId: string, options) => {
    try {
      const response = await api.get(`/api/v1/transactions/${txId}/raw`);
...
transactionCommand
  ...
  .action(async (rawTx: string) => {
    try {
      const response = await api.post("/api/v1/transactions/raw", {
...
transactionCommand
  ...
  .argument("<rawTx>", "Raw transaction hex string")
  .option("-f, --format <type>", "Output format (json/table)", "table")
  .action(async (rawTx: string, options) => {
    try {
      const response = await api.post("/api/v1/transactions/decode", {
        rawTransaction: rawTx,
      });
...
transactionCommand
  ...
  .action(async (options) => {
    try {
      const response = await api.post("/api/v1/transactions/estimate-fee", {
...
transactionCommand
  ...
  .action(async (options) => {
    try {
      const response = await api.post("/api/v1/transactions/sign-message", {
...
transactionCommand
  ...
  .requiredOption(
    "-s, --signature <string>",
    "Signature to verify (128 hex characters)"
  )
  .requiredOption(
    "-p, --public-key <string>",
    "Public key in hex format (130 characters)"
  )
  .action(async (options) => {
    try {
      const response = await api.post("/api/v1/transactions/verify-message", {
        message: options.message,
...
transactionCommand
  ...
  .action(async (options) => {
    try {
      const response = await api.post("/api/v1/transactions/validate-address", {
