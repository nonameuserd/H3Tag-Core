...
class HybridCrypto {
    static async sign(message, privateKey) {
        try {
            const privKey = typeof privateKey.privateKey === 'function'
    ...
    static async verify(message, signature, publicKey) {
        try {
            ...
            const eccValid = this.TRADITIONAL_CURVE
                .keyFromPublic(publicKey.address, 'hex')
    ...
    static async generateSharedSecret(input) {
        try {
            ...
            throw new HybridError(error instanceof Error ? error.message : 'Shared secret generation failed');
        }
    }
    static getMetrics() {
        return { ...this.metrics };
    ...
    static async deriveAddress(data) {
        try {
            ...
            const addressStr = typeof data.address === 'function' ? await data.address() : data.address;
