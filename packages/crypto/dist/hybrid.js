...
class HybridCrypto {
    static async sign(message, privateKey) {
        try {
            const resolvedPrivateKey = typeof privateKey === 'function'
                ? await privateKey()
                : privateKey;
            const eccKey = this.TRADITIONAL_CURVE.keyFromPrivate(resolvedPrivateKey, 'hex');
            ...
            const [dilithiumHash, kyberSecret] = await Promise.all([
                ...
                kyber_1.Kyber.encapsulate(resolvedPrivateKey).then((result) => result.sharedSecret),
            ]);
    ...
    static async decrypt(encryptedData, privateKey) {
        try {
            ...
            const resolvedPrivateKey = typeof privateKey === 'function'
                ? await privateKey()
                : privateKey;
            const kyberSecret = await kyber_1.Kyber.decapsulate(parsed.sessionKey, resolvedPrivateKey);
            const quantumKey = await quantum_wrapper_1.QuantumWrapper.hashData(Buffer.from(parsed.sessionKey), resolvedPrivateKey);
    ...
    static async verifyQuantumSignature(publicKey, signature, data, algorithm) {
        try {
            switch (algorithm) {
                ...
                case 'kyber':
                    return await kyber_1.Kyber.verify(signature, publicKey, data);
                default:
