...
class QuantumNative {
  constructor() {
    ...
    try {
      ...
      shared_1.Logger.error(
        'Failed to initialize quantum native module:',
        error,
      );
  ...
  async generateDilithiumKeyPair(entropy) {
    ...
    try {
      ...
      shared_1.Logger.debug(
        `Key pair generated in ${perf_hooks_1.performance.now() - start}ms`,
      );
      return result;
    } catch (error) {
  ...
  async kyberGenerateKeyPair() {
    ...
    try {
      ...
      shared_1.Logger.debug(
        `Kyber key pair generated in ${perf_hooks_1.performance.now() - start}ms`,
      );
      return result;
    } catch (error) {
