...
class QuantumWrapper {
  static async initialize() {
    ...
    await Promise.all([
      dilithium_1.Dilithium.generateKeyPair(), // Initialize Dilithium
  ...
  static async verify(message, signature, publicKey) {
    try {
      ...
      return await hybrid_1.HybridCrypto.verify(
        message.toString(),
  ...
  static async hashData(data) {
    try {
      ...
      return Buffer.from(
        hash_1.HashUtils.sha256(classicalHash + quantumHash.toString('hex')),
      );
    } catch (error) {
