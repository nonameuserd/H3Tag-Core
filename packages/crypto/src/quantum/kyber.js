...
class Kyber {
  ...
  static async generateKeyPair() {
    ...
    try {
      const keyPair =
        await _1.QuantumCrypto.nativeQuantum.kyberGenerateKeyPair();
  ...
  static async encapsulate(publicKey) {
    ...
    try {
      ...
      const result =
        await _1.QuantumCrypto.nativeQuantum.kyberEncapsulate(publicKeyBuffer);
      if (
        !result?.ciphertext ||
  ...
  static async decapsulate(ciphertext, privateKey) {
    ...
    try {
      ...
      const sharedSecret =
        await _1.QuantumCrypto.nativeQuantum.kyberDecapsulate(
          ciphertextBuffer,
          privateKeyBuffer,
        );
