...
class HybridCrypto {
  static async sign(message, privateKey) {
    try {
      ...
      const eccSignature = eccKey.sign(hash_1.HashUtils.sha256(message));
      const [dilithiumHash, kyberSecret] = await Promise.all([
        dilithium_1.Dilithium.hash(Buffer.from(message)),
        kyber_1.Kyber.encapsulate(privKey).then(
          (result) => result.sharedSecret,
        ),
      ]);
      return {
        address: hash_1.HashUtils.sha3(
  ...
  static async verify(message, signature, publicKey) {
    try {
      ...
      const eccValid = this.TRADITIONAL_CURVE.keyFromPublic(
        publicKey.address,
        'hex',
      ).verify(hash_1.HashUtils.sha256(message), signature.address);
      if (!eccValid) return false;
      // 3. Generate hybrid verification hash
      const verificationHash = hash_1.HashUtils.sha3(
        signature.address +
  ...
  static async encrypt(message, publicKey) {
    try {
      ...
      const sessionKey = crypto_js_1.default.lib.WordArray.random(
        this.KEY_SIZE / 8,
      );
      const { ciphertext: kyberCiphertext, sharedSecret: kyberSecret } =
        await kyber_1.Kyber.encapsulate(publicKey.address);
      // 2. Generate quantum-safe components
      const [dilithiumHash, quantumKey] = await Promise.all([
        dilithium_1.Dilithium.hash(Buffer.from(message)),
        quantum_wrapper_1.QuantumWrapper.hashData(
          Buffer.from(sessionKey.toString()),
        ),
      ]);
      // 3. Combine all secrets for encryption
      const hybridKey = hash_1.HashUtils.sha3(
        sessionKey.toString() +
          kyberSecret +
          dilithiumHash +
  ...
  static async decrypt(encryptedData, privateKey) {
    try {
      ...
      const kyberSecret = await kyber_1.Kyber.decapsulate(
        parsed.sessionKey,
        privateKey.address,
      );
      const quantumKey = await quantum_wrapper_1.QuantumWrapper.hashData(
  ...
  static async generateSharedSecret(input) {
    try {
      ...
      const { sharedSecret: quantumSecret } = await kyber_1.Kyber.encapsulate(
        kyberPair.publicKey,
      );
      ...
      const classicalSecret = this.TRADITIONAL_CURVE.genKeyPair().derive(
        this.TRADITIONAL_CURVE.keyFromPublic(
          hash_1.HashUtils.sha256(input.toString()),
          'hex',
        ).getPublic(),
      );
      // Combine secrets
      return hash_1.HashUtils.sha3(
        quantumSecret + classicalSecret.toString('hex'),
      );
    } catch (error) {
  ...
  static async deriveAddress(data) {
    try {
      ...
      const quantumHash = await quantum_wrapper_1.QuantumWrapper.hashData(
        Buffer.from(addressStr),
      );
      const combinedHash = hash_1.HashUtils.sha3(
      ...
      const checksum = hash_1.HashUtils.doubleSha256(
  ...
  static async calculateHybridHash(data) {
    try {
      ...
      const quantumHash = await dilithium_1.Dilithium.sign(
        JSON.stringify(data),
        privateKey,
      );
  ...
  static async hash(data) {
    try {
      ...
      const [dilithiumHash, kyberHash] = await Promise.all([
        ...
        kyber_1.Kyber.encapsulate(publicKey).then(
          (result) => result.sharedSecret,
        ),
      ]);
      // Combine all hashes
  ...
  static async generateRandomBytes(length) {
    try {
      return Buffer.from(
        crypto_js_1.default.lib.WordArray.random(length).toString(),
        'hex',
      );
    } catch (error) {
