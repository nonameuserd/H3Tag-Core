...
type PrivateKeyType = string | (() => Promise<string>);
  ...
  static async sign(
    ...
    try {
      ...
      const [dilithiumHash, kyberSecret] = await Promise.all([
        ...
        Kyber.encapsulate(resolvedPrivateKey).then((result) => result.sharedSecret),
      ]);
  ...
  public static async verifyQuantumSignature(
    ...
    try {
      switch (algorithm) {
        ...
        case 'kyber':
          return await Kyber.verify(signature, publicKey, data);
        default:
