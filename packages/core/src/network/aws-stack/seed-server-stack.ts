...
import { NodeInitializer } from "./script/node-initializer";
...
export class SeedServerStack extends cdk.Stack {
  private readonly merkleTree = new MerkleTree();
  ...
  private initialize(props: SeedServerStackProps): void {
    ...
    userData.addCommands(
      ...
      `echo '${JSON.stringify(
        blockchainConfig
      )}' > /opt/blockchain/config/blockchain-config.json`,

      // System updates and installation
      'apt-get update || { echo "Failed to update package list"; exit 1; }',
      'DEBIAN_FRONTEND=noninteractive apt-get upgrade -y || { echo "Failed to upgrade packages"; exit 1; }',
      'DEBIAN_FRONTEND=noninteractive apt-get install -y curl git build-essential || { echo "Failed to install dependencies"; exit 1; }',

      // Create initialization script
      "cat > /opt/blockchain/init.js << EOL\n" +
        NodeInitializer.getInitializationScript(blockchainConfig) +
        "\nEOL",
      ...
      `echo 'export NETWORK_TYPE=${this.config.networkType}' >> /etc/environment`,
      `echo 'export PORT=${BLOCKCHAIN_CONSTANTS.CURRENCY.NETWORK.port.MAINNET}' >> /etc/environment`,
      `echo 'export MAX_PEERS=125' >> /etc/environment`,
      `echo 'export MIN_PEERS=8' >> /etc/environment`,
      `echo 'export CONNECTION_TIMEOUT=${BLOCKCHAIN_CONSTANTS.UTIL.VALIDATION_TIMEOUT_MS}' >> /etc/environment`,

      // Configure worker pool
      `echo 'export MAX_WORKERS=${os.cpus().length}' >> /etc/environment`,
      `echo 'export WORKER_IDLE_TIMEOUT=60000' >> /etc/environment`,
      `echo 'export WORKER_HEALTH_CHECK_INTERVAL=30000' >> /etc/environment`
    );
    ...
    new route53.CfnHealthCheck(this, "SeedHealthCheck", {
      healthCheckConfig: {
        ...
        type: "TCP",
        resourcePath: "/health",
        fullyQualifiedDomainName: `${process.env.SEED_RECORD_NAME || "seed1"}.${
          process.env.DOMAIN_NAME || "h3tag.network"
        }`,
        requestInterval: 30,
        failureThreshold: 3,
      },
