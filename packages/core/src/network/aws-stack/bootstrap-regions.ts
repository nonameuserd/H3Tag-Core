...
if (!account) {
  console.error(
    '[Bootstrap] CDK_DEFAULT_ACCOUNT environment variable is required',
  );
  process.exit(1);
}
...
if (regions.length === 0) {
  console.error(
    '[Bootstrap] AWS_REGIONS environment variable is required and must contain valid regions',
  );
  process.exit(1);
}
...
if (invalidRegions.length > 0) {
  console.error(
    `[Bootstrap] Invalid AWS regions detected: ${invalidRegions.join(', ')}`,
  );
  process.exit(1);
}
