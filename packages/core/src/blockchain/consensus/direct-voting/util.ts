...
export class DirectVotingUtil {
  private readonly chainVoteMutex = new Mutex();
  ...
  constructor(
    ...
    this.metrics = new MetricsCollector("chain_selection");
  ...
  private async processVotingResults(
    ...
    try {
      ...
      if (
        approvalRatio >= BLOCKCHAIN_CONSTANTS.MINING.CHAIN_DECISION_THRESHOLD
      ) {
