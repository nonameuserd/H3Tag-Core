...
class DirectVotingUtil {
    ...
    constructor(db, auditManager) {
        ...
        this.metrics = new metrics_collector_1.MetricsCollector("node_selection");
    ...
    async initializeChainVotingPeriod(oldChainId, newChainId, forkHeight) {
        ...
        if (currentHeight - forkHeight >
            constants_1.BLOCKCHAIN_CONSTANTS.MINING.MAX_FORK_DEPTH) {
    ...
    async collectVotes(period, validators) {
        ...
        return this.voteMutex.runExclusive(async () => {
            if (Date.now() < periodSnapshot.endTime &&
    ...
    async processVotingResults(tally, oldChainId, newChainId) {
        ...
        try {
            ...
            if (approvalRatio >= constants_1.BLOCKCHAIN_CONSTANTS.MINING.NODE_SELECTION_THRESHOLD) {
