...
exports.AdvancedGPUMiner = void 0;
 ...
 */
class AdvancedGPUMiner extends gpu_1.GPUMiner {
    constructor() {
        super(...arguments);
     ...
     */
    calculateOptimalWorkgroupSize() {
        // Adjust based on GPU architecture
        return Math.min(this.maxComputeUnits * 32, // 32 threads per compute unit
     ...
     */
    async createOptimizedPipeline(target) {
        ...
        try {
            ...
            const shader = `
                ...
                const REWARD_PRECISION: u32 = ${this.CURRENCY_CONSTANTS.REWARD_PRECISION}u;
                const REWARD_SCALE: f32 = ${1.0 / this.CURRENCY_CONSTANTS.REWARD_PRECISION}f;
     ...
     */
    async mineOptimized(blockBuffer, target) {
        // Split work into optimal chunks
        const chunks = Math.ceil(this.MAX_NONCE / (this.workgroupSize * this.maxComputeUnits));
        const commands = [];
