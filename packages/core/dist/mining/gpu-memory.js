...
exports.AdaptiveGPUMemory = void 0;
/**
...
class AdaptiveGPUMemory {
    constructor() {
        this.MAX_MEMORY_USAGE = 0.8; // 80% of available GPU memory
    ...
    findAvailableBuffer(type, size) {
        const pool = this.memoryPools.get(type);
        if (!pool)
            throw new Error(`No memory pool found for type: ${type}`);
    ...
    async releaseMemory(type, buffer) {
        if (!buffer || !type)
            return;
