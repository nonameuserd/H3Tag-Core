...
class SeedServerStack extends cdk.Stack {
    ...
    initialize(props) {
        ...
        securityGroup.addEgressRule(ec2.Peer.anyIpv4(), ec2.Port.udp(53), "Allow DNS UDP outbound");
        // Allow IPv6 support
        securityGroup.addIngressRule(ec2.Peer.anyIpv6(), ec2.Port.tcp(8333), "Allow blockchain network traffic IPv6");
    ...
    setupInstanceConfiguration(instance, props) {
        ...
        'snap install amazon-ssm-agent --classic', 'systemctl enable amazon-ssm-agent', 'systemctl start amazon-ssm-agent', 
        // Set environment variables
        `echo 'export NODE_ENV=${props.environment}' >> /etc/environment`, `echo 'export MAX_WORKERS=${os.cpus().length}' >> /etc/environment`);
