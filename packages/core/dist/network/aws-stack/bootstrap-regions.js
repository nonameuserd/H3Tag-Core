...
const regions = process.env.AWS_REGIONS?.split(",").map((region) => region.trim()).filter(Boolean) || [];
...
regions.forEach((region) => {
    ...
    try {
        (0, child_process_1.execSync)(`cdk bootstrap aws://${account}/${region}`, {
            stdio: "inherit",
            timeout: 300000,
            env: {
                ...process.env,
        ...
        console.error(`[Bootstrap] Failed to bootstrap ${region}:`, error instanceof Error ? error.message : error);
    }
});
