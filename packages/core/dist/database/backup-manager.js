...
class BackupManager {
    ...
    async compressDatabase(backupPath) {
        ...
        try {
            ...
            if (backupDirCreated) {
                await fs_1.promises.rm(backupPath, { recursive: true, force: true }).catch(err => shared_1.Logger.error('Failed to cleanup after compression error:', err));
            }
            throw error;
        }
    }
    ...
    async createChecksum(path) {
        ...
        for (const file of files.sort()) { // Sort for consistency
    ...
    async getLatestBackup() {
        try {
            ...
            return sortedBackups.length ? (0, path_1.join)(this.config.backupPath, sortedBackups[0]) : null;
        }
        catch (error) {
    ...
    async verifyBackup(backupPath) {
        ...
        try {
            ...
            const metadataExists = await fs_1.promises.access(metadataPath)
                .then(() => true)
                .catch(() => false);
            if (!metadataExists) {
    ...
    async cleanOldBackups() {
        try {
            ...
            await Promise.all(sortedBackups
